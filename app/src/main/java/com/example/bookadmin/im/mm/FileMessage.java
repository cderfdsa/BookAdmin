...
public class FileMessage extends Message {
    ...
    public String getSummary() {
        return BookApplication.getInstance().getString(R.string.summary_file);
    }
    ...
    public void save() {
        ...
        if (SDCardUtils.isFileExist(filename, Environment.DIRECTORY_DOWNLOADS)) {
            Toast.makeText(BookApplication.getInstance(), "文件已存在",Toast.LENGTH_SHORT).show();
            return;
        }

        e.getToFile(SDCardUtils.getCacheFilePath(filename), new TIMCallBack() {
            @Override
