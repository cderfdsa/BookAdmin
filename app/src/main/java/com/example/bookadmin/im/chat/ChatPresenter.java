package com.example.bookadmin.im.chat;
...
public class ChatPresenter implements Observer {
     ...
     * 发送消息
     */
    public void sendMessage(int userAction, final String msg) {

        final TIMMessage message = changeMsg(userAction, msg);
            sendTIMMessage(message, new TIMValueCallBack<TIMMessage>() {
                @Override
                public void onError(int code, String desc) {//发送消息失败
                    //错误码code和错误描述desc，可用于定位请求失败原因
                    //错误码code含义请参见错误码表
                    LogUtils.i(String.format("send message onError: code = %d,msg = %s", code, desc));
                    view.onSendMessageFail(code, desc, message);
                }
                ...
                public void onSuccess(TIMMessage msg) {
                    //发送消息成功,消息状态已在sdk中修改，此时只需更新界面
                    LogUtils.i("send message onSuccess: ");
                    MessageEvent.getInstance().onNewMessage(null);
                }
            });
            //message对象为发送中状态
     ...
     *
     */
    public void sendOnlineMessage(int userAction, String msg){
    ...
    private TIMMessage changeMsg(int userAction, String msg){
        LogUtils.i( "sendMessage: userAction = " + userAction + " msg = " + msg);
        JSONObject json = new JSONObject();
        try {
            json.put("userAction", userAction);
            json.put("userId", UserInfo.getInstance().getUserId());
            json.put("nickname", UserInfo.getInstance().getNickname());
            json.put("headPic", UserInfo.getInstance().getHeadPic());
            if (msg != null) {
                json.put("params", msg);
            }
