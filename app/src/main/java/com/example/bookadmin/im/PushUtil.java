...
public class PushUtil implements Observer {
    ...
    private void PushNotify(TIMMessage msg){
        ...
        NotificationManager mNotificationManager = (NotificationManager) BookApplication.getInstance().getSystemService(BookApplication.getInstance().NOTIFICATION_SERVICE);
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(BookApplication.getInstance());
        Intent notificationIntent = new Intent(BookApplication.getInstance(), MainActivity.class);
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(BookApplication.getInstance(), 0,
...
//                .setNumber(++pushNum) //设置通知集合的数量
    ...
    public void reset(){
        NotificationManager notificationManager = (NotificationManager) BookApplication.getInstance().getSystemService(BookApplication.getInstance().NOTIFICATION_SERVICE);
