...
public class BookDeserializerOut implements JsonDeserializer<OrderOut> {
    ...
    @Override
    public OrderOut deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
...
//            }
        ...
        orderOut.setRp_id(rp_id);//订单id
        orderOut.setRp_name(rp_name);//：订单号
        orderOut.setRp_bcode(rp_bcode);//：订单状态码
        orderOut.setL1_region(l1_region);//:书库归属省
        orderOut.setL2_region(l2_region);//:书库归属市
        orderOut.setL3_region(l3_region);//:书库归属区
        orderOut.setL_address(l_address);//":书库详细地址,
        orderOut.setL_name(l_name);//":书库名称,
        orderOut.setBz_sn(bz_sn);//":采用包装sn
        orderOut.setB1_region(b1_region);//":书箱省
        orderOut.setB2_region(b2_region);//":书箱市
        orderOut.setB3_region(b3_region);//":书箱区
        orderOut.setAd_name(ad_name);//":书箱详细地址
        orderOut.setGs_name(gs_name);//":书箱名称编号
        orderOut.setU_name(u_name);//":用户名称
        orderOut.setBc_name(bc_name);//":状态名称
        orderOut.setRp_intime(rp_intime);//":下单时间（秒）,
        orderOut.setRp_uptime(rp_uptime);//":修改时间
        orderOut.setDate_find(date_find);// Date_find date_find;
        orderOut.setPhoto(photos);
